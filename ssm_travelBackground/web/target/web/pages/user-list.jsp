									...
									<c:forEach items="${pageInfo.list}" var="user">
										<tr>
											...
											<td class="text-center">
												...
												<a href="${pageContext.request.contextPath}/user/userUpRole.do?id=${user.id}" class="btn bg-olive btn-xs">添加角色</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<ul class="pagination">
								...
								<c:forEach begin="1" end="${pageInfo.pages}" var="user">
									<c:if test="${pageInfo.pageNum == user}">
										<li class="active">
											<a href="${pageContext.request.contextPath}/user/findAll.do?page=${user}&size=${pageInfo.pageSize}">${user}</a>
										</li>
									</c:if>
									<c:if test="${pageInfo.pageNum != user}">
										<li>
